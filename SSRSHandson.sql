...
Select C.CastID,C.CastCharacterName,F.FilmID,F.FilmName,A.ActorID,A.ActorName from tblCast C
join tblActor A on C.CastActorID=A.ActorID
join tblFilm F on F.FilmID=C.CastFilmID
Order by FilmName
...
select C.CastID,C.CastCharacterName,A.ActorID,A.ActorName,A.ActorGender,F.FilmID,F.FilmName from tblCast C
join tblActor A on C.CastActorID=A.ActorID
join tblFilm F on F.FilmID=C.CastFilmID
where FilmName like '%Casino%'
Order by FilmName,ActorGender


	select [FilmName],[FilmID],[CountryName],[CountryID],[LanguageName],[LanguageID],
	isnull([FilmBoxOfficeDollars],0) [FilmBoxOfficeDollars]
	from [tblFilm] F join 
	tblCountry C on C.CountryID=F.FilmCountryID
	join tblLanguage L on F.FilmLanguageID=L.LanguageID
	where LanguageName='Japanese'
	Order by LanguageName,CountryName
...
select  from dbo.Film where FilmID in (1679,1680)upper(Title) like '%ROBIN HOOD%'

SET IDENTITY_INSERT dbo.Film ON
INSERT dbo.Film 
( Title, ReleaseDate, DirectorID, StudioID, Review, CountryID, LanguageID, GenreID, RunTimeMinutes, CertificateID, BudgetDollars, BoxOfficeDollars, OscarNominations, OscarWins) 
VALUES (N'Robin Hood: Prince inserted 2', CAST(N'1975-06-14T00:00:00.000' AS DateTime), 17, 21, NULL, 241, 1, 8, 155, 2, 48000000, 390493908, 1, 0)

SET IDENTITY_INSERT dbo.Film OFF

SELECT        A.ActorID, R.RoleID, F.FilmID, F.Title, 
A.FullName, R.Role, F.ReleaseDate
FROM            Actor A INNER JOIN
                         dbo.Role R ON A.ActorID = R.ActorID INNER JOIN
                         Film F ON R.FilmID = F.FilmID

						 select top 100 * from Film where title like '%jurassic%'

						 use movies
						 go

						 Select gender,count(actorid),left(Year(DoB),3)  from Actor
						 Where DoB is not nUll
						 group by gender ,left(Year(DoB),3)
						 having gender='Male'

						 
						 Select gender,count(actorid) from Actor
						 Where DoB is not nUll
						 group by gender ,left(Year(DoB),3)
						 having gender='Male'
						 100 600
						1100	4507
						select title,max(budgetDollars)/1000000 budgetDollars ,max(boxofficedollars)/1000000  boxofficedollars from (
						SELECT        Film.FilmID, Film.Title, Role.Role, Film.ReleaseDate, Actor.FullName, Film.RunTimeMinutes, Film.BudgetDollars, Film.BoxOfficeDollars, Film.OscarNominations, Film.OscarWins
FROM            Film INNER JOIN
                         Role ON Film.FilmID = Role.FilmID INNER JOIN
                         Actor ON Role.ActorID = Actor.ActorID
WHERE        (Film.GenreID = 14)) a group by title
order by 1

select 2500000/1000000,70000000/1000000

SELECT        Actor.FullName, Actor.DoB, Film.FilmID
FROM            Actor INNER JOIN
                         Role ON Actor.ActorID = Role.ActorID INNER JOIN
                         Film ON Role.FilmID = Film.FilmID

						 select FilmID,Title from Film
						 where FilmID in (select distinct FilmID from Role) and FilmID=1
